...
class AddUser extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)->subject($username.' invited you to '.$this->team->name)->from(config('mail.from.address'), config('app.name').': '.$this->team->name)->markdown('mail.team.add-user', ['team' => $this->team, 'username' => $username]);
    }
