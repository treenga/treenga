...
class Commented extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        ->subject($username.' commented '.$this->category->name)
        ->from(config('mail.from.address'), config('app.name').': '.$this->team->name)
        ->markdown('mail.category.commented', ['category' => $this->category, 'username' => $username, 'team' => $this->team, 'link' => app()->make('url')->to('/team/'.$this->team->id.'/category/'.$this->category->id)]);
    }
