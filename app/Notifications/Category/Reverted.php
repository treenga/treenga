...
class Reverted extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        ->subject($username.' reverted description of '.$this->category->name)
        ->from(config('mail.from.address'), config('app.name').': '.$this->team->name)
        ->markdown('mail.category.reverted', ['category' => $this->category, 'username' => $username, 'team' => $this->team, 'link' => app()->make('url')->to('/team/'.$this->team->id.'/category/'.$this->category->id)]);
    }
